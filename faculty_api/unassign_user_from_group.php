...
if (!$groupId || !$assignmentType) {
    http_response_code(400);
    echo json_encode(['error' => 'Missing required parameters: group_id, assignment_type']);
    exit;
}
...
try {
    ...
    // Unassign user from group (set to NULL)
    $stmt = mysqli_prepare($dbConn, "UPDATE groups SET {$column} = NULL WHERE id = ?");
    if ($stmt === false) {
        throw new Exception("Prepare failed: " . mysqli_error($dbConn));
    }
    mysqli_stmt_bind_param($stmt, 'i', $groupId);
    ...
    if (mysqli_affected_rows($dbConn) > 0) {
        ...
        echo json_encode(['error' => 'Failed to unassign user']);
    }
